...
bool IsMaxInColN(int** a, const int rowNo, const int value, const int colCount, int colNo)
{
    if (colNo >= colCount)
        return true;
    if (a[rowNo][colNo] < value)
        ...
        return IsMaxInColN(a, rowNo, value, colCount, colNo + 1);
}

bool IsMinInRowK(int** a, const int colNo, const int value, const int rowCount, int rowNo)
{
    if (rowNo >= rowCount)
        return true;
    if (a[rowNo][colNo] > value)
        ...
        return IsMinInRowK(a, colNo, value, rowCount, rowNo + 1);
}

void Part2_SaddlePointRow(int** a, const int rowCount, const int colCount,
    int& No, const int rowNo, int colNo)
{
    if (IsMaxInColN(a, rowNo, a[rowNo][colNo], colCount, 0) &&
        IsMinInRowK(a, colNo, a[rowNo][colNo], rowCount, 0))
    {
